...
public class ElementList extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response) 
    ...
    try {
      ...
      String queryString = prefix+ "SELECT ?name ?number ?symbol {?element table:name ?name. ?element table:atomicNumber ?number. ?element table:symbol ?symbol.} ORDER BY ?number";
      ...
      try {
        ...
        out.println("<h3>Periodic Table Elements</h3>");
        while(rs.hasNext()){
		 ...
		 symbol=symbol.substring(0,symbol.indexOf("^"));
          ...
          out.println("<P><a href=\"http://tinman.cs.gsu.edu:8080/vgalimelu1/servlet/ElementDetails?val="+name+"\">"+name+"&nbsp("+symbol+")</a>");
        }
        out.println("</BODY>");
        out.println("</HTML>");
      } finally { 
